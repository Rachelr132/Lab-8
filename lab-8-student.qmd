...
missing_summary_map <- map_int(fish, ~sum(is.na(.))) %>% #count NAs for each column
...
missing_weight <- trout_data %>% #Filter rows with missing weight
  filter(is.na(weight))


missing_counts <- missing_weight %>%#Count missing values by year, section, and trip
  count(year, section, trip)
```

```{r figure}
ggplot(missing_counts, aes(x = trip, y = n, fill = section)) +
...
ggplot(fish, aes(x = length_scaled)) + 
  geom_histogram(binwidth = 0.05, fill = "darkorange") +
  labs(x = "Rescaled Values of Fish Length (0–1)", title = "Rescaled Length") +
  scale_y_continuous(limits = c(0, 4000)) +
  theme_minimal()
